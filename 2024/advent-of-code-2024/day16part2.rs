...
// $ cp-batch day16part2 | diff day16part2.out -
// $ cargo run --bin day16part2
...
fn solve() -> i64 {
    ...
    let mut prevs = vec![vec![vec![HashSet::new(); 4]; m]; n];
    ...
    while let Some((Reverse(score), (i, j), dir)) = q.pop() {
        ...
        if a[ii][jj] != '#' {
            if score + 1 < dist[ii][jj][dir] {
                dist[ii][jj][dir] = score + 1;
                q.push((Reverse(score + 1), (ii, jj), dir));
                prevs[ii][jj][dir].clear();
            }
            if score + 1 <= dist[ii][jj][dir] {
                prevs[ii][jj][dir].insert((i, j, dir));
            }
        }
        ...
        for delta in [-1isize, 1] {
            ...
            if score + 1000 < dist[i][j][dir2] {
                ...
                prevs[i][j][dir2].clear();
            }
            if score + 1000 <= dist[i][j][dir2] {
                prevs[i][j][dir2].insert((i, j, dir));
            }
        }
    }

    let last_dir = dist[di][dj]
        .iter()
        .enumerate()
        .map(|(x, y)| (y, x))
        .min()
        .unwrap()
        .1;
    let mut viz = vec![vec![vec![false; MOVES.len()]; m]; n];
    dfs(di, dj, last_dir, &mut viz, &prevs);

    // for i in 0..n {
    //     for j in 0..m {
    //         print!(
    //             "{}",
    //             if viz[i][j].iter().any(|&x| x) {
    //                 'O'
    //             } else {
    //                 a[i][j]
    //             }
...
fn dfs(
    ...
    j: usize,
    dir: usize,
    viz: &mut Vec<Vec<Vec<bool>>>,
    prev: &Vec<Vec<Vec<HashSet<(usize, usize, usize)>>>>,
) {
    if viz[i][j][dir] {
        return;
    }
    viz[i][j][dir] = true;
    for &(i, j, dir) in &prev[i][j][dir] {
        dfs(i, j, dir, viz, prev);
    }
}
