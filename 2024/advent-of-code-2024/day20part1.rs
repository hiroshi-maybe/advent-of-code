...
// $ cp-batch day20part1 | diff day20part1.out -
// $ cargo run --bin day20part1
...
fn solve() -> i64 {
    ...
    for i in 0..n {
        for j in 0..m {
            if a[i][j] == '.' {
                for mo in MOVES {
                    let ii = i.wrapping_add_signed(mo.0 * 2);
                    let jj = j.wrapping_add_signed(mo.1 * 2);

                    if ii < n && jj < m && a[ii][jj] != '#' {
                        let org = dist[ii][jj];
                        let cheat = dist[i][j] + 2;
